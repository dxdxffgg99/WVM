...
static void encode_instruction(uint8_t opcode, uint8_t mode, uint8_t dst, uint8_t src1, uint8_t src2, int64_t imm, uint8_t* out, size_t* pos) {
    if (out) {
        out[(*pos)++] = opcode;
        out[(*pos)++] = mode;
        out[(*pos)++] = dst;
    ...
    while (line != NULL) {
        ...
        if (op != -1) {
            ...
            switch (op) {
                ...
                case OR: case AND: case XOR: case LSH: case RSH:
                    if (t_op1[0] == '$') {
                        imm = parse_immediate(t_op1, &is_imm);
                        mode = ADDR_MODE_IMM;
                        if (imm > 0xFFFFFFFFLL || imm < -2147483648LL) mode |= ADDR_MODE_IMM8;
                ...
                case JMP: case CALL:
                    ...
                    break;
                case JE: case JNE: case JL: case JG: case JLE: case JGE: case LOOP:
                    if (t_op1[0] == '$') {
                        imm = parse_immediate(t_op1, &is_imm);
                        mode = ADDR_MODE_IMM;
                    } else {
                ...
                case POP:
                    dst = (uint8_t)parse_register(t_op1);
                    break;
                case SETZ: case TIME: case DEBUG:
                    dst = (uint8_t)parse_register(t_op1);
                    break;
                case RET: case EOP:
                    break;
